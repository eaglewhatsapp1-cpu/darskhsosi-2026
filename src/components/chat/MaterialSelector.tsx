...
const MaterialSelector: React.FC<MaterialSelectorProps> = ({
  ...
  const getSelectedMaterialNames = () => {
    return materials.filter(m => selectedMaterials.includes(m.id)).map(m => m.file_name).join(', ');
  };
  if (loading) {
    return <div className="p-3 bg-muted/50 rounded-lg animate-pulse">
        <div className="h-4 bg-muted rounded w-1/2"></div>
      </div>;
  }
  if (materials.length === 0) {
    return <div className="p-3 bg-muted/30 rounded-lg text-center">
        <p className="text-sm text-muted-foreground">
          {t('لا توجد مواد مرفوعة. ارفع مواد تعليمية أولاً.', 'No materials uploaded. Upload learning materials first.')}
        </p>
      </div>;
  }
  return <Collapsible open={isOpen} onOpenChange={setIsOpen} className="border rounded-lg my-[32px]">
        ...
        <Button variant="ghost" className="w-full justify-between p-3 h-auto">
          <div className="flex items-center gap-2 text-start">
            ...
            <div>
              ...
              {selectedMaterials.length > 0 && <p className="text-xs text-muted-foreground truncate max-w-[300px]">
      ...
      <CollapsibleContent>
        <ScrollArea className="max-h-[200px] p-2">
          <div className="space-y-1">
            {materials.map(material => <div key={material.id} className="flex items-center gap-3 p-2 hover:bg-muted/50 rounded-md cursor-pointer transition-colors" onClick={() => handleToggle(material.id)}>
                <Checkbox checked={selectedMaterials.includes(material.id)} disabled={!selectedMaterials.includes(material.id) && selectedMaterials.length >= maxSelection} />
